...
Date        : 10/4/24
Description : Script to test the servo motor component

            When program starts, servo is centred for 120ms,
            rotates anticlockwise till at max, then rotates
            clockwise till max, then repeats process forever.

            At start, servo is set to centre, with checkSchedule()
            after 120ms raising flag for servo, which causes
            checkFlags() to turn the servo, then 120ms later
            checkSchedule() raises flag for servo again and process
            repeats.
--------------------------------------------------------------------------------
Functions Present
--------------------------------------------------------------------------------
...
Change History
--------------------------------------------------------------------------------
10-APR-2024 andrewlaw9178 created to modularise the servo 
11-APR-2024 andrewwlaw9178 changed code and found limits of component
			then changed code back to original purpose
12-APR-2024 andrewwlaw9178 updated comments
--------------------------------------------------------------------------------
*/
...
// Servo info (Port 2)
struct Servo servoA = {2, SERVO_PIN, (PWM_SERVO_UPPER-PWM_SERVO_LOWER)/NUMBER_OF_ANGLES_CHECKED, 0};    //PWM on Port 2.4, angle to be turned, initially turn anti-clockwise
...
//==============================================================================
// End of File :  HardwareTests/ServoTest.c
