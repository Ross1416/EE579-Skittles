...
Date        : 9/4/24
Description : Script to test the white button component

			When white button is pressed indicator LED toggles.

			When Port 1 interrupt is triggered by start stop button,
			flag is raised for button, which causes checkFlags() to 
			start a debounce timer, which checkSchedule() raises 
			debounce flag, which causes indicator LED to be toggled
			in checkFlags().
--------------------------------------------------------------------------------
Functions Present
--------------------------------------------------------------------------------
...
// White button 
struct Button WhiteButton = {2, WHITE_BUTTON};
...
int main(void)
{
    ...
    // Setup device
    setupWhiteButton(&WhiteButton);                                     // Selects white button input
...
__interrupt void Port1_ISR(void)
{
    ...
    P2IFG &= ~(WHITE_BUTTON);                                                  // Assuming that BIT3 is the WHITE_BUTTON and BIT4 is the
    return;
}
