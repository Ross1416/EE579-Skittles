...
#define MODE_SWITCH         BIT3                   					// Located at P1.1
...
void setupStartStop()
{
    //Setup button for input and interrupt (P1.1)
    P1DIR &= ~MODE_SWITCH;

    //Pull up so when pressed will go high to low
    P1REN |= MODE_SWITCH;
    P1OUT |= MODE_SWITCH;
    P1IE |= MODE_SWITCH;                                            // Enable interrupts
    P1IES |= MODE_SWITCH;                                           // High to Low transition
    P1IFG &= ~MODE_SWITCH;                                          // Clear interrupts
}
...
void checkFlags()
{
    ...
    if(flag.debounce)
    {
        if(P1IES && MODE_SWITCH)									// Edge is Low to High transition - so if input is to recieve 3V3 - so now check
		{
			if((P1IN & MODE_SWITCH) == MODE_SWITCH)            		// Button still pressed after debounce
			{
		...
		else
		{
			if((P1IN & MODE_SWITCH) != MODE_SWITCH)            		// Button still pressed after debounce
			{
				LEDIndicatorOff();                                   // Switch to search and destroy
			}
		}
        Schedule.debounce.sec = 0;
        Schedule.debounce.ms = -1;
...
__interrupt void Port1_ISR(void)
{
	...
	P1IES ^= MODE_SWITCH;												// Toggle edge such that when switch is moved it will trigger again 
    P1IFG &= ~(MODE_SWITCH);                                            // Assuming that BIT3 is the MODE_SWITCH and BIT4 is the
    return;
}
