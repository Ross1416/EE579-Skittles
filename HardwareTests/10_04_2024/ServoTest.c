...
At start, servo is set to centre, with checkSchedule() after 200ms raising flag for servo, which causes checkFlags() to turn the servo,
then 200 ms later checkSchedule() raises flag for servo again and process repeats.
*/
...
//  Define variable containing servo information
struct Servo{
    char pwmPin;
    unsigned int speed;
    char direction;
...
void checkFlags()
{
    ...
    if(flag.servoMove)
    {
        ...
        // Move servo after
        timeIncrement(&Schedule.turnServo, 0, 200);
...
int main(void)
{
    ...
    // Start servo at centre and wait
    servoCenter();

    // Set servo to start after 0.2s
    timeIncrement(&Schedule.turnServo, 0, 200);
