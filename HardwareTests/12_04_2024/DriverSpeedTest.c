...
When program starts, car will move forward for 2 seconds, then reverse backwards for 2 seconds. This will occur forever,
alternating between two speeds, with the LED Indicator used to show the faster speed.
...
void checkSchedule()
{
    ...
    if(isTime(Schedule.nextMovement))
    {
        ...
        else if(movementStage == 3)
        {
            ...
            LEDIndicatorOff();                   // Turn off the indicator LED connected to P2.5
        }
...
int main(void)
{
    ...
    setupLEDIndicator();                                                // Set up output port for LED Indicator
