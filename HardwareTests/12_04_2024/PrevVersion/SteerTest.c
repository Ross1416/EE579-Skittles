...
When program starts, cars front wheels will turn left for 0.5 seconds, then straight for 0.5 seconds, then right for 0.5
seconds, then straight for 0.5 seconds. This will occur forever.
...
in the checkSchedule() raises a flag every 0.5 seconds to move the front wheels to a different position, with checkFlags() making the
change.
