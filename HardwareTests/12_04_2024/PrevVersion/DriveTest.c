...
When program starts, car will move forward for 0.5 seconds, then reverse backwards for 0.5 seconds. This will occur forever,
alternating between two speeds, with the LED Indicator used to show the faster speed.

At start, PWM signal will be set up, with it using checkSchedule() to create a PWM signal by alternating the state. The other section
in the checkSchedule() raises a flag every 0.5 seconds to change the direction the back wheels move, with checkFlags() making the change.
*/
