/*
Filename    : HardwareTests/BlackButtonTest.c
Author      : Andrew Law
Project     : EE579 Skittles Knocking Over Project
Date        : 9/4/24
Description : Script to test the black button component

			When black button is pressed indicator LED toggles.

			When Port 1 interrupt is triggered by start stop 
			button, flag is raised for button, which causes 
			checkFlags() to start a debounce timer, which 
			checkSchedule() raises debounce flag, which causes 
			indicator LED to be toggled in checkFlags().
--------------------------------------------------------------------------------
Functions Present
--------------------------------------------------------------------------------
setupBlackButton()
...
// Defines structure containing all timings that events occur 
struct Scheduler {
...
// Start Stop button 
struct Button BlackButton = {1, BLACK_BUTTON};
...
void checkFlags()
{
    ...
    if(flag.debounce)
    {
        if((P1IN & BLACK_BUTTON) != BLACK_BUTTON)             			// Button still pressed after debounce
        {
...
int main(void)
{
    ...
    // Setup device
    setupBlackButton(&BlackButton);                                     // Selects black button input
...
__interrupt void Port1_ISR(void)
{
    ...
    P2IFG &= ~(BLACK_BUTTON);                                                  // Assuming that BIT3 is the BLACK_BUTTON and BIT4 is the
    return;
}
