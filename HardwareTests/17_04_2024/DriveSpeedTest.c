/*
Filename    : HardwareTests/DriveSpeedTest.c
Author      : Andrew Law
Project     : EE579 Skittles Knocking Over Project
Date        : 11/4/24
Description : Script to test the back DC motor and H-Bridge components
			and ensure different speeds can occur
			
			When program starts, car will move forward for 2 seconds, 
			then reverse backwards for 2 seconds. This will occur 
			forever, alternating between 5 different speeds, with the  
			indicator LED used to show the fastest speed.
...
Change History
--------------------------------------------------------------------------------
11-APR-2024 andrewlaw9178 created to modularise the driving
...
#define SPEED_TOP           0.95*MOTOR_PWM_PERIOD                        //Top speed of the motors as percentage of PWM.
#define SPEED_UPPER         0.8*MOTOR_PWM_PERIOD
#define SPEED_MIDDLE        0.6*MOTOR_PWM_PERIOD
#define SPEED_LOWER         0.5*MOTOR_PWM_PERIOD
...
void checkSchedule()
{
    ...
    if(isTime(Schedule.initiallyMoveForward))
    {
        ...
        indicatorLEDOn(&indicatorLED);                   			// Turn on the indicator LED
    ...
    if(isTime(Schedule.nextMovement))
    {
        ...
        {
            // Stop driving forwards upper range of speed
            motorDrive.direction = FORWARD;
            flag.motorDrive = 1;
            motorDrive.pwm.aMs = SPEED_UPPER;
            indicatorLEDOff(&indicatorLED);                   		// Turn off the indicator LED
        }
        ...
        {
            // Start driving forwards middle range of speed
            motorDrive.direction = FORWARD;
            flag.motorDrive = 1;
            motorDrive.pwm.aMs = SPEED_MIDDLE;
        }
		else if(movementStage == 8)
        {
            // Stop car
			motorDrive.direction = OFF;
            flag.motorDrive = 1;
        }
		else if(movementStage == 9)
        {
            // Start driving backward
			motorDrive.direction = BACK;
            flag.motorDrive = 1;
        }
		else if(movementStage == 10)
        {
            // Stop car
			motorDrive.direction = OFF;
            flag.motorDrive = 1;
        }
		else if(movementStage == 11)
        {
            // Start driving car forwards lower range of speed
			motorDrive.direction = FORWARD;
            flag.motorDrive = 1;
			motorDrive.pwm.aMs = SPEED_MIDDLE;
        }
		else if(movementStage == 12)
        {
            // Stop car
			motorDrive.direction = OFF;
            flag.motorDrive = 1;
        }
		else if(movementStage == 13)
        {
            // Start driving backward
			motorDrive.direction = BACK;
            flag.motorDrive = 1;
        }
		else if(movementStage == 14)
        {
            // Stop car
			motorDrive.direction = OFF;
            flag.motorDrive = 1;
        }
		else if(movementStage == 15)
        {
            // Start driving car forwards lowest speed
			motorDrive.direction = FORWARD;
            flag.motorDrive = 1;
			motorDrive.pwm.aMs = SPEED_SLOW;
        }
		else if(movementStage == 16)
        {
            // Stop car
			motorDrive.direction = OFF;
            flag.motorDrive = 1;
        }
		else if(movementStage == 17)
        {
            // Start driving backwards
			motorDrive.direction = BACK;
            flag.motorDrive = 1;
        }
		else if(movementStage == 18)
        {
            // Stop car
			motorDrive.direction = OFF;
            flag.motorDrive = 1;
        }
		else if(movementStage == 19)
        {
            // Start driving car 
			motorDrive.direction = FORWARD;
            flag.motorDrive = 1;
...
//==============================================================================
// End of File :  HardwareTests/DriveSpeedTest.c
