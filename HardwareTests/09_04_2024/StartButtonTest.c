...
__interrupt void Port1_ISR(void)
{
    ...
    P1IFG &= ~(START_STOP_SWITCH);													// Assuming that BIT3 is the START_STOP_SWITCH and BIT4 is the 
    return;
}
...
void checkFlags()
{
    ...
    if(flag.debounce)
    {
        if((P1IN & START_STOP_SWITCH) != START_STOP_SWITCH)   									//Button still pressed after debounce ---------------------------------- either 0x08 or BIT3 or START_STOP_SWITCH choose!
        {
...
void setupButton()
{
    ...
    P1DIR &= ~START_STOP_SWITCH;
	
    //Pull up so when pressed will go high to low
    P1REN |= START_STOP_SWITCH;
    P1OUT |= START_STOP_SWITCH;
    P1IE |= START_STOP_SWITCH;    												// Enable interrupts
    P1IES |= START_STOP_SWITCH;   												// High to Low transition
    P1IFG &= ~START_STOP_SWITCH;  												// Clear interrupts
}
