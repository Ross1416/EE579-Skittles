...
__interrupt void Port2_ISR(void)
{
    ...
    P2IFG &= ~(WHITE_BUTTON);													// Assuming that BIT3 is the switch and BIT4 is the 
    return;
}
...
void checkFlags()
{
    ...
    if(flag.debounce)
    {
        if((P1IN & WHITE_BUTTON) != WHITE_BUTTON)   									//Button still pressed after debounce ---------------------------------- either 0x08 or BIT3 or SWITCH choose!
        {
...
void setupButton()
{
    ...
    P2REN |= WHITE_BUTTON;
    P2OUT |= WHITE_BUTTON;
    P2IE |= WHITE_BUTTON;    												// Enable interrupts
    P2IES |= WHITE_BUTTON;   												// High to Low transition
    P2IFG &= ~WHITE_BUTTON;  												// Clear interrupts
}
