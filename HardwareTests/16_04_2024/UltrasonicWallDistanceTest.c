...
struct flags {
	...
	// Read right wall ultrasonic when result avialable
...
unsigned int  wallDistancesLeft[WALL_READINGS] = {[0 ... WALL_READINGS-1] = 0};	// Initalise distance readings to left wall as all 0s
unsigned int  wallDistancesRight[WALL_READINGS] = {[0 ... WALL_READINGS-1] = 0};	// Initalise distance readings to right wall as all 0s
int ultraUsed;															// Selects which ultrasonic will be used
...
void checkFlags()
{
	...
	if (flag.ultraLeftWallRead)
    {
        ...
        {
            wallDistancesLeft[i] = wallDistancesLeft[i - 1]; 			// Shift each value along hence dropping oldest value
        }
        wallDistancesLeft[0] = ultraLeft.distance;						// Newest value is added
		...
		if(wallDistancesLeft[0] < dist2pulse(50))
		{
			LEDIndicatorOn();
		}
		else
	...
	if (flag.ultraRightWallRead)
    {
        ...
        for(i = WALL_READINGS-1; i > 0; i--)
        {
            wallDistancesRight[i] = wallDistancesRight[i - 1]; 			// Shift each value along hence dropping oldest value
        }
        wallDistancesRight[0] = ultraRight.distance;					// Newest value is added
		...
		if(wallDistancesRight[0] < dist2pulse(50))
		{
			LEDIndicatorOn();
		}
		else
...
int main(void)
{
	...
	// Select left ultrasonic wall sensor
	ultraUsed = 0;
